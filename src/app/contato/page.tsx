    ...
    <>
      <section className="relative py-20 overflow-hidden">
        ...
        <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          <h1 className="text-4xl sm:text-5xl font-cinzel text-gold-gradient mb-4">Fale Conosco</h1>
          <div className="gothic-divider w-24 mx-auto mb-4" />
          <p className="text-text-secondary max-w-xl mx-auto">Escolhe o canal que preferir. Pagamento via Pix 💰</p>
        </div>
      </section>
      ...
      <section className="py-20 bg-diablo-black">
        <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {canais.map((canal) => (
              <div key={canal.title} className="glass-card rounded-xl p-8 text-center group">
                ...
                <a href={canal.href} target="_blank" rel="noopener noreferrer"
                  className={`inline-flex items-center gap-2 px-6 py-3 bg-gradient-to-r ${canal.color} text-white font-semibold rounded-lg hover:shadow-lg transition-all btn-glow`}>
                  {canal.action}
                </a>
              </div>
            ))}
      ...
      <section className="py-20 bg-diablo-dark">
        <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-12">
            <h2 className="text-2xl sm:text-3xl font-cinzel text-gold-gradient mb-4">FAQ</h2>
            <div className="gothic-divider w-20 mx-auto" />
          </div>
          <div className="space-y-3">
            {faq.map((item) => (
              <details key={item.q} className="group glass-card rounded-lg overflow-hidden">
                <summary className="p-4 cursor-pointer flex items-center justify-between hover:bg-diablo-stone/30 transition-colors">
                  ...
                  <svg className="w-4 h-4 text-diablo-gold group-open:rotate-180 transition-transform" fill="none" stroke="currentColor" viewBox="0 0 24 24">
